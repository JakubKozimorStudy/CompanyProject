...
void DatabaseRepository::saveEmployee(Employee newEmp, MYSQL* conn) {
	...
	std::string insert_query = "INSERT INTO employee (id, first_name, last_name, role_id, salary) VALUES (' " + std::to_string(lastId + 1) + "', '" + newEmp.getFirstName() + "', '"
		+ newEmp.getLastName() + "', '" + std::to_string(newEmp.getRole()) + "', '" + std::to_string(newEmp.getSalary()) + " ')";
